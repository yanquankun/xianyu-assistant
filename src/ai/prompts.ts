...
const DESCRIPTION_POLISH_CONSTRAINT = [
  ...
  '后续每行只写一项已明确提供的信息，使用“字段：内容”的格式，不使用项目符号或 Markdown 列表。',
...
const DESCRIPTION_POLISH_FINAL_REMINDER = [
  ...
  '严格保持逐行格式：第一行只写整理后的商品名称，后续每行只写一项已知事实并使用“字段：内容”的格式；不得改写成连续段落。',
...
export function buildDescriptionPolishMessages(
  ...
  return [
    ...
    {
      role: 'user',
      content: `请根据以下当前表单中的全部可用商品信息进行二手场景润色扩写。所有字段都可以作为文案依据，但不得把“来源平台”写入结果。严格采用第一行商品名称、后续每行“字段：内容”的格式，只输出商品描述正文。\n\n${currentFormInformation}`
    }
  ];
}
